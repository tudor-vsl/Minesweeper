...
def unveil_check_matrix(check_matrix, unveil_Matrix, flag_Matrix, main_Matrix):
    ...
    for i in range(0, rows):
        x = indent
        for j in range(0, cols):
            if check_matrix[i][j] == 1 and flag_Matrix[i][j] == 0 and main_Matrix[i][j] != 9 and unveil_Matrix[i][j] == 0:
...
def main():
    ...
    while notQuitGame:
        ...
        if width // 2 + box_w // 2 > x_mouse > width // 2 - box_w // 2 and height - header + box_h > y_mouse > height - header - box_h + 30:
            pygame.draw.rect(window, bright_red,(width // 2 - box_w // 2, height - header + box_h // 2 / 3, box_w, box_h))
            if click_intro[0] == 1:
                notQuitGame = False
